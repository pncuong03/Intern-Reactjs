...
const Modal: React.FC<ReactModal.Props & { closeModal?: () => void; title?: string; bgColor: string }> = ({
  ...
  return (
    <ReactModal
      {...props}
      className={clsx(
        'absolute top-[180px] left-1/2 w-[500px] max-w-[calc(100%_-_32px)] -translate-x-1/2 overflow-hidden rounded-2xl outline-none',
        props.className
      )}
    ...
    >
      {title && (
        <div className={clsx('flex justify-between p-4 border-b-[2px]', bgColor)}>
          <h2 className='text-[24px] font-semibold text-neutral-100'>{title}</h2>
